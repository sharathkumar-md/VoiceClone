  ...
  python cli.py generate \\
    ...
    for p in [generate_parser, narrate_parser]:
        ...
        p.add_argument(
            ...
            help='Device for TTS (default: cuda)'
        )
        p.add_argument(
            '--exaggeration',
            type=float,
    ...
    if args.command == 'text-only':
        try:
            ...
            print(f"Word count: {len(result['story'].split())} words")
    ...
    # Initialize narrator
    try:
        print("Initializing Story Narrator...")
        ...
        print(f"Error initializing narrator: {e}")
        return
    ...
    try:
        ...
        if not args.quiet:
            ...
            print("SUCCESS!")
            print("="*60)
            print(f"Audio saved to: {result['audio']['output_path']}")
            print(f"Duration: {result['audio']['duration_seconds']:.1f} seconds")
