...
if __name__ == "__main__":
    ...
    print("Processed Story Chunks:")
    print("=" * 50)
    for chunk in result["chunks"]:
        print(f"[Chunk {chunk.chunk_id}] {chunk.text}")
        print(f"  - Paragraph end: {chunk.is_paragraph_end}, Pause: {chunk.pause_after}s")
        print()
    
    print("Metadata:")
    print(f"Total chunks: {result['metadata']['total_chunks']}")
    print(f"Total words: {result['metadata']['total_words']}")
    print(f"Estimated duration: {result['metadata']['estimated_duration_seconds']:.1f} seconds")
