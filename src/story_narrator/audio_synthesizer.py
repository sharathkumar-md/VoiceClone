...
class AudioSynthesizer:
    """
    Handles text-to-speech synthesis using Chatterbox TTS
    """
    ...
    def set_voice(self, voice_sample_path: str, exaggeration: float = None):
        ...
        print(f"Preparing voice conditionals from {voice_sample_path}...")
    ...
    def synthesize_chunks(
        ...
        for idx, (text, pause) in enumerate(zip(text_chunks, pause_durations)):
            ...
            # Add pause if needed
            if pause > 0:
                pause_samples = int(pause * self.tts_model.sr)
    ...
    def synthesize_and_save(
        ...
        """
        Complete synthesis pipeline: chunks → audio → save
        ...
        """
        print(f"\nSynthesizing {len(text_chunks)} chunks...")
        ...
        # Calculate duration
        duration_seconds = full_audio.shape[-1] / self.tts_model.sr
        
        print(f"Saving audio (duration: {duration_seconds:.1f}s)...")
