...
class StoryNarrator:
    ...
    def __init__(
        ...
        """
        print("Initializing Story Narrator system...")
        ...
        self.audio_synthesizer = AudioSynthesizer(device=device)
        
        print("✓ Story Narrator initialized successfully")
    ...
    def create_story_narration(
        ...
        if save_story_text:
            ...
            with open(text_output_path, 'w', encoding='utf-8') as f:
                f.write(story_text)
            if show_progress:
                print(f"✓ Story text saved to {text_output_path}")
        
        # Step 2: Process text
        if show_progress:
            print("\n" + "="*60)
            print("STEP 2: Processing text for TTS...")
            print("="*60)
        ...
        if show_progress:
            print(f"✓ Text processed into {len(chunks)} chunks")
            print(f"  Estimated duration: {processed['metadata']['estimated_duration_seconds']:.1f}s")
        
        # Step 3: Load voice and synthesize
        if show_progress:
            print("\n" + "="*60)
            print("STEP 3: Synthesizing narration with voice cloning...")
            print("="*60)
        ...
        if show_progress:
            ...
            print("="*60)
            print(f"Audio file: {audio_result['output_path']}")
            print(f"Duration: {audio_result['duration_seconds']:.1f}s")
            print(f"Chunks synthesized: {audio_result['num_chunks']}")
    ...
    def narrate_existing_story(
        ...
        if show_progress:
            print("\n" + "="*60)
            print("Narrating existing story...")
            print("="*60)
