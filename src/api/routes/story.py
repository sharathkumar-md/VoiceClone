...
async def generate_story(request: StoryGenerateRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Note: Stories table doesn't have user_id column yet, so we can't associate with users
        try:
            saved_story = StoryService.create_story(
                ...
                metadata={
                    ...
                    "user_id": user["id"],  # Store user_id in metadata for now
                    "username": user["username"],
                }
            )
            logger.info(f"✓ Story saved to database: {story_id} (generated by {user['username']})")
...
async def edit_story(story_id: str, request: StoryEditRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        logger.info(f"Story found: {story.id}, proceeding with edit")
